    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-1">
          ...
          {CATEGORY_PILLS.map((category) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all duration-200 active:scale-95 cursor-pointer ${
                  isSelected
                    ? "bg-gradient-to-r from-indigo-600 via-indigo-500 to-cyan-500 text-white font-bold border border-indigo-400/60 shadow-[0_0_20px_rgba(99,102,241,0.4)] ring-1 ring-white/20"
                    : "bg-slate-800/40 border border-slate-700/50 hover:border-indigo-400 hover:bg-slate-800/80 text-slate-300 hover:text-white backdrop-blur-md"
                }`}
              >
                <Icon className={`w-3.5 h-3.5 ${isSelected ? "text-white animate-pulse" : "text-slate-400"}`} />
                <span>{category.display}</span>
                {isSelected && (
                  <span className="w-1.5 h-1.5 rounded-full bg-cyan-300 shadow-[0_0_6px_rgba(6,182,212,0.8)]" />
                )}
              </button>
            );
          })}
        ...
        <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
          <div className="inline-flex items-center gap-2 text-xs font-medium text-slate-300 bg-slate-900/60 backdrop-blur-md px-3.5 py-1.5 rounded-xl border border-slate-800">
          ...
          {isFiltered && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-cyan-400 hover:text-cyan-300 transition-colors py-1.5 px-2.5 hover:bg-cyan-950/40 rounded-xl border border-cyan-500/20 cursor-pointer"
